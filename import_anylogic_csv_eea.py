...
class ImportfromAnyLogicEEA:
    
    def __init__(self, uncertainties, outcomes, policies, number_of_experiments, era_duration):
        self.uncertainties = uncertainties
        self.outcomes = outcomes
        self.policies = policies
        self.nexp = number_of_experiments
        self.era = era_duration
    ...
    def extract_experiments(self):
        experiments = np.zeros(self.nexp*len(self.policies), dtype={'names':self.uncertainties+['policy'], 'formats':['float64']*len(self.uncertainties)+['a64']})
    ...
    def extract_outcomes(self):
        ...
        for outcome in self.outcomes:
            for i, val in enumerate(self.policies):
                ...
                df_outcome = df_outcome.append(df_outcome.iloc[[-1]*(self.era-len(df_outcome.index))])
